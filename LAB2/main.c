...
void t_resultat_ecrire(t_resultat** resultat_ptr, const char* nom_fichier)
{
    ...
    int nb_titres = get_nb_titres(resultat_ptr);
	...
	for (unsigned int i = 0; i < nb_titres; i++) {
        fprintf(fichier_ptr, "%s\t %s\t %s\t %s\t %s\n", resultat_ptr[i].titre.tconst, 
                                                         resultat_ptr[i].titre.titleType,
                                                         resultat_ptr[i].titre.primaryTitle,
                                                         resultat_ptr[i].titre.startYear,
                                                         resultat_ptr[i].titre.genres);
    }
